        ...
        {windowWidth >= 735 ? (
          <div className='md:w-1/2 flex flex-col gap-8 justify-center md:z-50'>
            {Steps.map((item, index) => {
              return (
                <div key={index} className='max-w-[800px] h-screen flex items-end justify-center [&:nth-child(3)]:items-center'>
          ...
          <>
            {progression > 0 && progression < 1 && (
              <div className='gap-8  md:z-50'>
                {Steps.map((item, index) => {
                  console.log(progression, item.progress);
                  return (
                    <div
                      key={index}
            ...
            <div className='md:w-1/2 flex flex-col gap-8 justify-center md:z-50'>
              {Steps.map((item, index) => {
                return (
                  <div key={index} className='max-w-[800px] h-screen flex items-end justify-center [&:nth-child(3)]:items-center'>
                    <div className='flex gap-4 items-start justify-center'>
                      <span className='text-black/30 text-[40px] italic leading-[52.4px] md:mt-4 text-transparent'>{`0${index + 1}`}</span>
                      <div className='flex flex-col gap-2'>
                        ...
                        <p className=' text-[16px] leading-[20.4px] sm:text-[20px] sm:leading-[26.2px] text-transparent'>{item.description}</p>
                      </div>
                    </div>
                  </div>
                );
